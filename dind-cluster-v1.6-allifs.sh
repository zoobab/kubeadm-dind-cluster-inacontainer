...
function dind::init {
  ...
  local container_id=$(dind::run kube-master "${master_ip}" 1 0.0.0.0:${APISERVER_PORT}:8080 ${master_opts[@]+"${master_opts[@]}"})
...
function dind::wait-for-ready {
  ...
  "${kubectl}" get nodes >&2
  dind::step "Access dashboard at:" "http://0.0.0.0:${APISERVER_PORT}/ui"
}
...
function dind::restore {
  ...
  for ((n=0; n <= NUM_NODES; n++)); do
    (
      if [[ n -eq 0 ]]; then
        dind::step "Restoring master container"
        dind::restore_container "$(dind::run -r kube-master "${master_ip}" 1 0.0.0.0:${APISERVER_PORT}:8080 ${master_opts[@]+"${master_opts[@]}"})"
